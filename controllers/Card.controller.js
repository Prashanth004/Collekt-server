...
exports.product_export = function(req, res, nect){
    ...
    Card.find({ session_id: req.user._id }).lean().exec({}, function(err, products) {
    ...
    if (!req.user) {
        ...
        Card.find({ user_id: req.user._id, unq_name:req.body.unq_name}, function (err, product) {
            ...
            if (product.length == 0) {
                ...
                Card.findOneAndUpdate({session_id: req.user._id, unq_name:req.body.unq_name }, { $set: { "why": req.body.why } }, function (err) {
                    if (err) return res.status(400).send(err);
...
exports.cardInfo = function(req,res,next){
    ...
    if (!req.user) {
        ...
        Card.find({ session_id: req.user._id, domain: req.body.domain, unq_name:req.body.unq_name }, function (err, product) {
            if (err) res.status(400).send(err);
            if (product.length > 0) {
                res.status(200).send({
                    collected : 1,
